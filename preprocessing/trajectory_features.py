...
def calculate_climb_features(trajectory):
    ...
    for name, series in all_series.items():
        features.update(stats_for_series(series, f"climb_{name}"))
        for ds in [5, 10, 30, 60]:
            series = series[delta_t <= ds] * 60
            features.update(stats_for_series(series, f"climb_{name}_first_{ds}s"))

    return features


def calculate_traj_features(trajectory, flight_info):
    features = {}
    adep = flight_info["adep"]
    dep = get_near_airport_traj(trajectory, adep)
    if len(dep) == 0:
...
def add_trajectory_features(challenge):
    for date in tqdm(challenge.df.date.unique()):
        ...
        for fid in tqdm(flight_ids):
            ...
            features = calculate_traj_features(trajs[trajs.flight_id == fid], fid_info)
