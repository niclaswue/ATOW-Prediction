...
trajectory_data_file =  additional_data_dir / "all_trajectory_features.parquet"
...
class TrajectoryPreprocessor(BasePreprocessor):
    ...
    def process(self, dataset: Dataset) -> Dataset:
        ...
        if not trajectory_data_file.exists():
            print("TrajectoryPreprocessor: Trajectory data not found. Running batch processing script to create the features.")
        ...
        if not set(dataset.df["flight_id"]).issubset(set(trajectory_features["flight_id"])):
            print("TrajectoryPreprocessor: Not all flight_ids in the dataset are in the trajectory features. Running batch processing script again to create the features.")
            create_trajectory_features_batch()
            trajectory_features = pd.read_parquet(trajectory_data_file)
